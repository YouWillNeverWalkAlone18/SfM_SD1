...
// ========== 練習試行用　ブロック関数 (無作為な刺激) ==========
function makePracticeBlock(numTrials) {
  let trials = [];
  
  // 自刺激種類を同じ比率で混ぜる (CW, CCW, Neutral)
  // 20回を3で割ると丁度割り切れないため、リストを作ってシャッフル
  const stim_types = jsPsych.randomization.repeat([sfm_cw, sfm_ccw, sfm_neutral], Math.ceil(numTrials / 3)).slice(0, numTrials);

  for (let i = 0; i < numTrials; i++) {
    let trial_sketch = stim_types[i];
    ...
    trials.push({
      ...
      data: {
        ...
        trial_in_block: i,
        stimulus_type: trial_sketch === sfm_cw ? 'sfm_cw' : (trial_sketch === sfm_ccw ? 'sfm_ccw' : 'sfm_neutral')
      }
    });
    ...
    trials.push({
      ...
      questions: [{
        prompt: '<div style="color: #e0e0e0; font-size : 26px; line-height : 1.4; margin-bottom: 60px;">【練習】今の刺激では、回転方向が迷いなく一方向に見えていましたか？</div>',
        name: 'practice_conf_dir',
        labels: [
          `<span style="position: relative; color: #e0e0e0;"><span style="position: absolute; left: -220px; top: 0; width: 200px; text-align: right; color: #aaa; font-size: 16px;">（両方の方向が見えた）</span>全くそう思わない</span>`,
          '<span style="color: #e0e0e0;">あまりそう思わない</span>',
          '<span style="color: #e0e0e0;">ややそう思う</span>',
          '<span style="color: #e0e0e0;">かなりそう思う</span>',
          `<span style="position: relative; color: #e0e0e0;">非常にそう思う<span style="position: absolute; right: -240px; top: 0; width: 220px; text-align: left; color: #aaa; font-size: 16px;">（1方向だけが見えていた）</span></span>`
        ],
        required: true
      }],
      data: { task: 'practice_likert' }
    });
...
　　　</div>',
    ...
    // new trial push - slider response
    trials.push({
  ...
  prompt: '<div style="color: #e0e0e0; font-size : 26px; line-height : 1.4; margin-bottom: 60px;">今の刺激では、回転方向が明確に一方向に見えていましたか？</div>',
  ...
  on_finish: function(data) {
    // slider raw value (0–100)만 저장
    data.raw_slider = data.response;

    // 전체 실험 기준 response trial index
    data.trial_index_global =
